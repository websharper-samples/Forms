...
{
 "use strict";
 var Global,Samples,PersonPets,WebSharper,Operators,Person,Forms,Form,JavaScript,JS,Seq,Pet,Strings,Pervasives,Validation,Species,Enumerator,Obj,T,UI,Doc,View,HtmlModule,attr,View$1,Submitter,Many,Collection,Doc$1,ItemOperations,CollectionWithDefault,Client,Templates,Pervasives$1,Fresh,Var$1,List,T$1,Result,Arrays,Unchecked,Snap,AttrProxy,ErrorMessage,Var,Attr,EventTarget,Node,Collections,Dictionary,DomUtility,ConcreteVar,Docs,Attrs,AttrModule,Array,List$1,SC$1,HashSet,WindowOrWorkerGlobalScope,SC$2,SC$3,Abbrev,Fresh$1,DocElemNode,CharacterData,Object,Elt,DictionaryUtil,Prepare,Slice,KeyCollection,An,Settings,Mailbox,Fresh$2,Attrs$1,Dyn,Updates,Docs$1,RunState,NodeSet,Anims,SC$4,SC$5,Queue,DynamicAttrNode,SC$6,SC$7,Concurrency,AppendList,HashSetUtil,Easing,AsyncBody,SC$8,CT,HashSet$1,Scheduler,CancellationTokenSource,DomNodes,Error,OperationCanceledException,Lazy,SC$9,LazyExtensionsProxy,LazyRecord,IntelliFactory,Runtime,console,$,Date;
 Global=self;
 Samples=Global.Samples=Global.Samples||{};
 PersonPets=Samples.PersonPets=Samples.PersonPets||{};
 ...
 {
  var a;
  a=Form.Render(Runtime.Curried(PersonPets.RenderPerson,4),Form.Run(function(p)
  {
   Global.alert("Welcome to you "+p.firstName+" "+p.lastName+Strings.concat("",Seq.map(function(pet)
   {
    return", your "+Global.String(pet.species)+" "+pet.name;
   },p.pets))+"!");
 ...
 {
  return Form.WithSubmit(Pervasives.op_LessMultiplyGreater(Pervasives.op_LessMultiplyGreater(Pervasives.op_LessMultiplyGreater(Form.Return(Runtime.Curried3(Person.New)),Validation.IsNotEmpty("Please enter your first name.",Form.Yield(init.firstName))),Validation.IsNotEmpty("Please enter your last name.",Form.Yield(init.lastName))),Form.Many$1(init.pets,Pet.New(Species.Cat,""),PersonPets.PetPiglet)));
 };
 PersonPets.RenderPerson=function(firstName,lastName,pets,submit)
 {
  return Doc.Element("section",[AttrProxy.Create("class","section")],[Doc.Element("h2",[AttrProxy.Create("class","subtitle")],[Doc.TextNode("You")]),Doc.Element("div",[AttrProxy.Create("class","field")],[Doc.Element("label",[AttrProxy.Create("class","label")],[Doc.TextNode("First name: "),Doc.Input([AttrProxy.Create("class","input")],firstName)]),PersonPets.ShowErrorsFor(View$1.Through$1(submit.view,firstName))]),Doc.Element("div",[AttrProxy.Create("class","field")],[Doc.Element("label",[AttrProxy.Create("class","label")],[Doc.TextNode("Last name: "),Doc.Input([AttrProxy.Create("class","input")],lastName)]),PersonPets.ShowErrorsFor(View$1.Through$1(submit.view,lastName))]),Doc.Element("h2",[AttrProxy.Create("class","subtitle")],[Doc.TextNode("Your pets")]),Doc.Element("div",[],[pets.Render(Runtime.Curried3(function(ops,species,name)
  {
   return Doc.Element("div",[AttrProxy.Create("class","field is-horizontal")],[Doc.Element("div",[AttrProxy.Create("class","field-body")],[PersonPets.RenderPet(species,name),Doc$1.ButtonValidate("Move up",[AttrProxy.Create("class","button")],ops.moveUp),Doc$1.ButtonValidate("Move down",[AttrProxy.Create("class","button")],ops.moveDown),Doc.Button("Delete",[AttrProxy.Create("class","button")],function()
   {
    ops["delete"]();
   }),PersonPets.ShowErrorsFor(View$1.Through$1(submit.view,name))])]);
  })),Doc.Button("Add a pet",[AttrProxy.Create("class","button")],function()
  {
   pets.AddOne();
 ...
 PersonPets.PetPiglet=function(init)
 {
  ...
  }),Form.Yield(init.species)),Validation.IsNotEmpty("Please enter your pet's name.",Form.Yield(init.name)));
 };
 ...
 PersonPets.RenderPet=function(species,name)
 {
  return Doc.Concat([Doc.Element("div",[AttrProxy.Create("class","field")],[Doc.Element("label",[AttrProxy.Create("class","label")],[Doc.Radio([],Species.Cat,species),Doc.TextNode(Global.String(Species.Cat))])]),Doc.Element("div",[AttrProxy.Create("class","field")],[Doc.Element("label",[AttrProxy.Create("class","label")],[Doc.Radio([],Species.Dog,species),Doc.TextNode(Global.String(Species.Dog))])]),Doc.Element("div",[AttrProxy.Create("class","field")],[Doc.Element("label",[AttrProxy.Create("class","label")],[Doc.Radio([],Species.Piglet,species),Doc.TextNode(Global.String(Species.Piglet))])]),Doc.Element("div",[AttrProxy.Create("class","field")],[Doc.Input([AttrProxy.Create("class","input")],name)])]);
 };
 ...
 Species=PersonPets.Species=Runtime.Class({
  toString:function()
  {
